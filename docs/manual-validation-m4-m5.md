...
tests + 7 `registry` inline tests + 12 e2e tests pin the
...
- `liquid-sdk-bridge::registry` inline unit tests: **7 passed;
  ...
  path resolution, plus an `io_err`-coverage test that triggers
  `fs::create_dir_all` failure by routing the registry root
  through a regular-file path — added alongside M6.5 to close
  the codecov gap on the `io_err` helper).
